        ...
        function() {
            ...
            describe('Нажимаю на кнопку "Добавить правило".', function() {
                ...
                describe('Заполняю форму.', function() {
                    ...
                    describe('Выбираю некий фильтр.', function() {
                        beforeEach(function() {
                            helper.editForm.combobox().withFieldLabel('Место потери звонка').option('Некий фильтр').
                                click();
                        });

                        describe(
                            'Снимаю отметку с чекбокс справа от поля "Не перезванивать, если абонент ожидал ответа ' +
                            'менее".',
                        function() {
                            beforeEach(function() {
                            ...
                            it(
                                'Отмечаю чекбокс справа от поля "Не перезванивать, если абонент ожидал ответа ' +
                                'менее". Поле "Не перезванивать, если абонент ожидал ответа менее" доступно и ' +
                                'заполнено значением.',
                            function() {
                                helper.minWaitDurationCheckbox().click();
                        ...
                        it(
                            'Оставляю отмеченным чекбокс справа от поля "Не перезванивать, если абонент ожидал ' +
                            'ответа менее". Нажимаю на кнопку "Сохранить". Правило сохранено.',
                        function() {
                            helper.editForm.textfield().
                                withFieldLabel('Не перезванивать, если абонент ожидал ответа менее').fill('15');
                            
                            helper.button('Сохранить').click();
                            helper.autoBackCallCreatingRequest().receiveResponse();
                        });
                    });
                    it('Выбираю фильтр по сценарию. Выбираю сценарий и операцию.', function() {
                        helper.editForm.combobox().withFieldLabel('Место потери звонка').option('Сценарий').click();

                        helper.editForm.combobox().withFieldLabel('Сценарий ВАТС').clickArrow();
                        helper.editForm.combobox().withFieldLabel('Сценарий ВАТС').
                            option('Перенаправление на 89265564454').click();

                        helper.editForm.combobox().withFieldLabel('Операция сценария').clickArrow();
                        helper.editForm.combobox().withFieldLabel('Операция сценария').
                            option('Переадресация 1').click();
                    });
                });
