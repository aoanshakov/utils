...
function JsTester_Loader (files, moduleNames) {
    return function (context, moduleName, url) {
        ...
        if (!files[url]) {
            throw new Error('Мок подключаемого файла "' + url + '" не зарегестророван.');
        }

        eval(files[url] + '//@ sourceURL=' + url);
...
function JsTester_RequireJsMock (files) {
    var realLoad = window.require.load,
        moduleNames = {},
        fakeLoad = new JsTester_Loader(files, moduleNames),
        realNextTick = window.require.nextTick,
        urlFetched = {},
        defQueueMap = {},
        defQueue = {},
        defined = {};
    ...
    function eachContext (callback) {
        var contextName;

        for (contextName in window.require.s.contexts) {
            callback(window.require.s.contexts[contextName], contextName);
        }
    }
    ...
    function mockContexts () {
        eachContext(function (context, contextName) {
            //context.urlFetched = copyObject(urlFetched[contextName]);
            //context.defQueueMap = copyObject(defQueueMap[contextName]);
            //context.defQueue = copyObject(defQueue[contextName]);
            //context.defined = copyObject(defined[contextName]);
            context.nextTick = fakeNextTick;
        });
    }

    function unmockContexts () {
        eachContext(function (context) {
            context.nextTick = realNextTick;
        });
    }

    eachContext(function (context, contextName) {
        urlFetched[contextName] = copyObject(context.urlFetched);
        defQueueMap[contextName] = copyObject(context.defQueueMap);
        defQueue[contextName] = copyObject(context.defQueue);
        defined[contextName] = copyObject(context.defined);
    });

    this.replaceByFake = function () {
        window.require.load = fakeLoad;

        var moduleName;

        for (moduleName in moduleNames) {
            window.require.undef(moduleName);
            delete(moduleNames[moduleName]);
        }

        mockContexts();
    };
    this.restoreReal = function () {
        window.require.load = realLoad;
        unmockContexts();
    };
}

function JsTester_RequiredFilesRegistrator (files) {
...
function JsTester_Tests (factory) {
    var testRunners = [],
        ...
        files = {},
        registerFiles = JsTester_RequiredFilesRegistrator(files),
        requireJsMocker = new JsTester_RequireJsMock(files),
    ...
    this.runTests = function () {
        ...
        var args = [
            ...
            userMedia, rtcConnectionsMock, navigatorMock, timeoutLogger, registerFiles
...
function JsTester_InputElement (
    ...
    function erase (updateBeforeCursor, updateAfterCursor, keyCode) {
        ...
        getDomElement().dispatchEvent(createKeyboardEvent('keydown', keyCode,
            function () {
                setValue = function () {};
                moveCursor = function () {};
            }));
        ...
        getDomElement().dispatchEvent(createKeyboardEvent('keyup', keyCode, function () {}));
