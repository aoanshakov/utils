function requestResponseFiles (responseFileNames, callback) {
    var noResponse = {};

    var responses = responseFileNames.reduce(function (responses, name) {
        responses[name] = noResponse;
        return responses;
    }, {});

    function maybeRunCallback () {
        !Object.values(responses).some(function (response) {
            return response == noResponse;
        }) && callback(responses);
    }

    Object.keys(responses).forEach(function (fileName) {
        ...
        request.open('GET', '/tests/js/helpers/responses/' + fileName + '.json');
        request.send();
    });
...
Ext.application({
    ...
    launch: function () {
        requestResponseFiles(tests.getResponseFileNames(), function (responses) {
            var viewport,
                me = this;
